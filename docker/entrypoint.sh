...
init_arches() {
	...
	if [[ ! -d ${APP_FOLDER} ]] || [[ ! "$(ls ${APP_FOLDER})" ]] || [[ ! -f ${APP_FOLDER}/manage.py ]] ; then
		echo ""
		...
		arches-admin startproject ${ARCHES_PROJECT}2 -y
		mv marine-weather2/* ${ARCHES_PROJECT}
		run_setup_db
...
start_celery_supervisor() {
	cd ${APP_FOLDER}
	supervisord -c docker/core_testing-supervisor.conf
}
...
run_setup_db() {
	...
	echo ""
	cd ${APP_FOLDER}
	python3.11 manage.py setup_db --force
}
...
run_load_package() {
	...
	echo ""
	cd ${APP_FOLDER}
	python3.11 manage.py packages -o load_package -s marine_weather/pkg -db -dev -y
}
...
run_django_server() {
	...
	cd ${APP_FOLDER}
    echo "Running Django"
	exec sh -c "pip install debugpy -t /tmp && python3.11 -Wdefault /tmp/debugpy --listen 0.0.0.0:5678 manage.py runserver 0.0.0.0:${DJANGO_PORT}"
}
...
run_livereload_server() {
    ...
    exec sh -c "python3.11 manage.py developer livereload --livereloadhost 0.0.0.0"
}
